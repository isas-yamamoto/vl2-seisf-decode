...
def build_map_table() -> list[int]:
    """
    512-slot MAT[] as generated by N51SUB MAP (values 1..512).

    The generating loop assigns mat[temp] = q + 512 for q running -511..0
    (512 assignments, values 1..512). Stopping the moment q reaches 0 (i.e.
    *before* assigning that final q=0 -> 512 entry) leaves exactly one 9-bit
    index -- whichever `temp` the recurrence lands on next -- at its
    zero-initialized default. `_mat_probe` returning 0 for that index used
    to be mapped to matv=503 (the identity case) by `estimate_q_from_matv`'s
    `matv <= 0` fallback; it is actually matv=512 (see
    draft/AUTHOR_NOTES.md item 22), which needs the general Q>503 segment
    formula (segment lengths 503, 9 per page), not the identity shortcut.
    This was the entire mechanism behind matv=503's ~48% archive-wide
    bit-exact failure rate: two distinct matv values (503 and 512) were
    colliding into one reported value.
    """
    mat = [0] * 512
    temp = 0
    q = -511
...
def _scramble_bits_via_map(scrambled: Sequence[int], inverse: bool = False) -> list[int]:
    ...
    for seq in range(512):
        ...
        addr = _ADDR_OF_SEQ[seq + 1] if seq + 1 <= 512 else seq
        if inverse:
...
def pd7400072_readout_order(q: int) -> list[int]:
    ...
    504-512 for the "greater than" case per the 9-bit field range), read
    ...
    both regimes). Q == 512 is included in the "greater than" range (not
    just 504-511): it is the one matv value the MAP table generator used to
    drop (see `build_map_table`, draft/AUTHOR_NOTES.md item 22); its segment
    lengths (503, 9) fall out of the same Q > 503 formula with no special
    case needed.
    """
    if q == 503:
...
def estimate_q_from_matv(halfwords: Sequence[int], frame_base: int) -> Tuple[int, int]:
    ...
    [1, 512] (Q == matv throughout: validated for 0 < matv < 503 against
    366 header-matched pairs; matv == 503 is the PD7400072 identity case;
    504 <= matv <= 512 uses the PD7400072 "Q > 503" formula -- see
    `pd7400072_readout_order`). matv == 512 was previously misread as 503
    by a now-fixed off-by-one in `build_map_table` (see its docstring and
    draft/AUTHOR_NOTES.md item 22); this was the entire mechanism behind
    matv=503's ~48% archive-wide bit-exact failure rate.
    """
    data_base = frame_base + SEISF_HEADER_HALFWORDS
    if data_base + 1 >= len(halfwords):
        return 0, 503
